const baseUrl =
  "https://newsapi.org/v2/top-headlines?apiKey=93238bcda39e4404852697d364b77971";
...
const newsLayout = newsitem => {
  ...
  const imageurl =
    newsitem.urlToImage !== null
      ? `${newsitem.urlToImage}`
      : "https://placeholdit.co//i/400x400?&bg=808069&fc=eee8cd&text=Project Image";
  const source =
  ...
  const description =
    newsitem.description !== null ? `<h3>${newsitem.description}</h3>` : "";
  const content =
    newsitem.content !== null
      ? `<p>${newsitem.content.split("[+")[0]}</p>`
      : "";
  ...
  return `
   ...
   <input class="redcard-checkbox" type="checkbox" name="ratenews"> <label>Show article the Red Card!</label>
    </section>`;
};

// TOFIX cleared on refresh, due to reset on displayDateOnPage?
function redCard() {
